...
*/
	...
	function moveToPGNString2($curColor, $piece, $fromRow, $fromCol, $toRow, $toCol, $pieceCaptured, $promotedTo, $isChecking)
	{
		...
		$tmpGameQ = $dbc->query("SELECT whitePlayer,blackPlayer,dateCreated,gameMessage FROM " . $CFG_TABLE['games'] . " WHERE gameID = " . $GameID) or die(mysql_error());
		$tmpGame = $tmpGameQ->fetch_array(MYSQLI_ASSOC);
		
		$gStart = $tmpGame['dateCreated'];
		$isDraw="";
		if($tmpGame['gameMessage']=="draw"){$isDraw=true;}else{$isDraw="";}
		
		$tmpBlackQ = $dbc->query("SELECT nick,firstName,lastName FROM " . $CFG_TABLE['players'] . " WHERE playerID = ".$tmpGame['blackPlayer']);
                $xBlack = $tmpBlackQ->fetch_array(MYSQLI_ASSOC);
                $pBlack = $xBlack['nick'];
		$pBlackF = $xBlack['firstName'];
		$pBlackL = $xBlack['lastName'];
     
	        $tmpWhiteQ = $dbc->query("SELECT nick,firstName,lastName FROM " . $CFG_TABLE['players'] . " WHERE playerID = ".$tmpGame['whitePlayer']);
                $xWhite = $tmpWhiteQ->fetch_array(MYSQLI_ASSOC);
                $pWhite = $xWhite['nick'];
