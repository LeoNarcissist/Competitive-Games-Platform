...
/* these functions deal specifically with undoing a move */
	function doUndo()
	{
		...
		$tmpMaxTime = $dbc->query("SELECT Max(timeOfMove) FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$_SESSION['gameID']);
		$maxTime = mysql_result($tmpMaxTime,0);
		$moves = $dbc->query("SELECT * FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$_SESSION['gameID']." AND timeOfMove = '$maxTime'");
		...
		if ($lastMove = mysql_fetch_array($moves, MYSQL_ASSOC))
		{
			/* if the last move was played by this player */
				...
				$numMoves--;
				$dbc->query("DELETE FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$_SESSION['gameID']." AND timeOfMove = '$maxTime'");
