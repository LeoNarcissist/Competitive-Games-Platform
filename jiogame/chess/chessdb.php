...
*/
	...
	function updateTimestamp()
	{
		...
		$dbc->query("UPDATE " . $CFG_TABLE['games'] . " SET lastMove = NOW() WHERE gameID = ".$_SESSION['gameID']);
	}
	...
	function loadHistory()
	{
		...
		$allMoves = $dbc->query("SELECT * FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$_SESSION['gameID']." ORDER BY timeOfMove");
	...
	function savePromotion()
	{
		...
		if ($CFG_USEEMAILNOTIFICATION && ! $_SESSION['isSharedPC'])
		{
			...
			if ($oppColor == 'white')
				$tmpOpponentID = $dbc->query("SELECT whitePlayer FROM " . $CFG_TABLE['games'] . " WHERE gameID = ".$_SESSION['gameID']);
			else
				$tmpOpponentID = $dbc->query("SELECT blackPlayer FROM " . $CFG_TABLE['games'] . " WHERE gameID = ".$_SESSION['gameID']);
			...
			if ($tmpOpponentEmail->num_rows > 0)
			{
				...
				{
					/* get opponent's nick */
					$tmpOpponentNick = $dbc->query("SELECT nick FROM " . $CFG_TABLE['players'] . " WHERE playerID = ".$_SESSION['playerID']);
					$opponentNick = mysql_result($tmpOpponentNick, 0);
					
					/* get opponent's prefered history type */
					$tmpOpponentHistory = $dbc->query("SELECT value FROM " . $CFG_TABLE['preferences'] . " WHERE playerID = ".$opponentID." AND preference = 'history'");
					
					/* default to PGN */
					if ($tmpOpponentHistory->num_rows > 0)
	...
	function loadGame()
	{
		...
		$pieces = $dbc->query("SELECT * FROM " . $CFG_TABLE['pieces'] . " WHERE gameID = ".$_SESSION['gameID']);
		...
		$tmpTurns = $dbc->query($tmpQuery);
		$tmpTurn = $tmpTurns->fetch_array(MYSQLI_ASSOC);
	...
	function saveGame()
	{
		...
		/* clear old data */
		$dbc->query("DELETE FROM " . $CFG_TABLE['pieces'] . " WHERE gameID = ".$_SESSION['gameID']);
		...
		for ($i = 0; $i < 8; $i++)
		{
			...
			for ($j = 0; $j < 8; $j++)
			{
				...
				if ($board[$i][$j] != 0)
				{
					...
					$dbc->query("INSERT INTO " . $CFG_TABLE['pieces'] . " (gameID, color, piece, row, col) VALUES (".$_SESSION['gameID'].", '$tmpColor', '$tmpPiece', $i, $j)");
				}
			}
		}
	...
	function processMessages()
	{
		...
		$tmpMessages = $dbc->query($tmpQuery);
		...
		if ($_POST['isCheckMate'] == 'true')
			$dbc->query("UPDATE " . $CFG_TABLE['games'] . " SET gameMessage = 'checkMate', messageFrom = '".$currentPlayer."' WHERE gameID = ".$_SESSION['gameID']);
		...
		$tmpMessages = $dbc->query($tmpQuery);
		$tmpMessage = $tmpMessages->fetch_array(MYSQLI_ASSOC);
