...
error_reporting(-1);
	...
	$tmpOldGames = $dbc->query($tmpQuery);

	/* for each older game... */
	while($tmpOldGame = $tmpOldGames->fetch_array(MYSQLI_ASSOC))
	{
		/* ... clear the history... */
		$dbc->query("DELETE FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$tmpOldGame['gameID']);

		/* ... and the board... */
		$dbc->query("DELETE FROM " . $CFG_TABLE['pieces'] . " WHERE gameID = ".$tmpOldGame['gameID']);

		/* ... and the messages... */
		$dbc->query("DELETE FROM " . $CFG_TABLE['messages'] . " WHERE gameID = ".$tmpOldGame['gameID']);

		/* ... and finally the game itself from the database */
		$dbc->query("DELETE FROM " . $CFG_TABLE['games'] . " WHERE gameID = ".$tmpOldGame['gameID']);
	}
	...
	switch($_POST['ToDo'])
	{
		case 'NewUser':
			...
			$existingUsers = $dbc->query($tmpQuery);
			...
			if ($dbc->query($tmpQuery))
				$_SESSION['playerID'] = $dbc->insert_id;
		...
		case 'Login':
			...
			$tmpPlayers = $dbc->query($tmpQuery);
			$tmpPlayer = $tmpPlayers->fetch_array(MYSQLI_ASSOC);
			...
			$tmpPreferences = $dbc->query($tmpQuery);
			...
			while($tmpPreference = $tmpPreferences->fetch_array(MYSQLI_ASSOC))
			{
		...
		case 'InvitePlayer':
			...
			$tmpExistingRequests = $dbc->query($tmpQuery);

			if ($tmpExistingRequests->num_rows == 0)
			{
				...
				if ($CFG_USEEMAILNOTIFICATION)
				{
					...
					$tmpOpponentEmail = $dbc->query("SELECT value FROM " . $CFG_TABLE['preferences'] . " WHERE playerID = ".$_POST['opponent']." AND preference = 'emailNotification'");
					if ($tmpOpponentEmail->num_rows > 0)
					{
		...
		case 'WithdrawRequest':
			...
			$tmpOpponentID = $dbc->query("SELECT whitePlayer FROM " . $CFG_TABLE['games'] . " WHERE gameID = ".$_POST['gameID']);
			if ($tmpOpponentID->num_rows > 0)
			{
				$opponentID = mysql_result($tmpOpponentID, 0);

				if ($opponentID == $_SESSION['playerID'])
				{
					$tmpOpponentID = $dbc->query("SELECT blackPlayer FROM " . $CFG_TABLE['games'] . " WHERE gameID = ".$_POST['gameID']);
					$opponentID = mysql_result($tmpOpponentID, 0);
				}
				...
				{
					/* if opponent is using email notification... */
					$tmpOpponentEmail = $dbc->query("SELECT value FROM " . $CFG_TABLE['preferences'] . " WHERE playerID = ".$opponentID." AND preference = 'emailNotification'");
					if ($tmpOpponentEmail->num_rows > 0)
					{
						$opponentEmail = mysql_result($tmpOpponentEmail, 0);
						if ($opponentEmail != '')
		...
		case 'UpdatePersonalInfo':
			...
			$tmpPassword = $dbc->query($tmpQuery);
			...
			<div class="preferences-form">
				...
				<div class="form-block">
						...
						<?php
							...
							$tmpPlayers = $dbc->query($tmpQuery);
							$first = true;
							while($tmpPlayer = $tmpPlayers->fetch_array(MYSQLI_ASSOC))
							{
								echo ('<option ');
								if($first)
								{
				...
				<div class="form-block">
						...
						<tbody id="respInviteTblBdy">
							<?php
								...
								$tmpGames = $dbc->query($tmpQuery);

								$rowNbr = 0;
								if ($tmpGames->num_rows == 0)
									echo("<tr><td colspan=\"3\">" . gettext("You are not currently invited to any games") . "</td></tr>\n");
								else
									while($tmpGame = $tmpGames->fetch_array(MYSQLI_ASSOC))
									{
										if($rowNbr %2 == 0)
										...
										/* get white's nick */
										$tmpPlayer = $dbc->query("SELECT nick FROM " . $CFG_TABLE['players'] . " WHERE playerID = ".$tmpGame['whitePlayer']);
										$player = mysql_result($tmpPlayer, 0);
										echo ('</td><td>');
										echo($player);

										/* black's nick */
										$tmpPlayer = $dbc->query("SELECT nick FROM " . $CFG_TABLE['players'] . " WHERE playerID = ".$tmpGame['blackPlayer']);
										$player = mysql_result($tmpPlayer, 0);
										echo ('</td><td>');
										echo($player);
		...
		<div class="preferences">
			<div class="preferences-form">
				...
				<div class="form-block">
					...
					<?php
						$tmpGames = $dbc->query("SELECT * FROM " . $CFG_TABLE['games'] . " WHERE gameMessage = '' AND (whitePlayer = ".$_SESSION['playerID']." OR blackPlayer = ".$_SESSION['playerID'].") ORDER BY dateCreated");
						...
						else
						{
							...
							while($tmpGame = $tmpGames->fetch_array(MYSQLI_ASSOC))
							{
								...
								$tmpNumMoves = $dbc->query("SELECT COUNT(gameID) FROM " . $CFG_TABLE['history'] . " WHERE gameID = ".$tmpGame['gameID']);
...
<?php
	$tmpGames = $dbc->query("SELECT * FROM " . $CFG_TABLE['games'] . " WHERE (gameMessage <> '' AND gameMessage <> 'playerInvited' AND gameMessage <> 'inviteDeclined') AND (whitePlayer = ".$_SESSION['playerID']." OR blackPlayer = ".$_SESSION['playerID'].") ORDER BY lastMove DESC");
